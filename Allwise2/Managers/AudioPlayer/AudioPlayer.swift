...
// Function to start playing music random choice
...
// Function to start playing music selected title
func startPlayingSound(soundname: String) {
    let fileName = soundname // Construct file name using the random number
    guard let url = Bundle.main.url(forResource: fileName, withExtension: "mp3") else { return }
    do {
        audioPlayer = try AVAudioPlayer(contentsOf: url)
        audioPlayer?.numberOfLoops = -1 // Loop indefinitely
